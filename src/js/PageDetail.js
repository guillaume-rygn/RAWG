...
const PageDetail = (argument) => {
  const preparePage = () => {
    ...
    const displayGame = (gameData) => {
      ...
      articleDOM.querySelector("p.store").innerHTML = stores.map(store => `<a href="https://www.${store.store.domain}" target="_blank" style="margin:0"><span style="color:white">${store.store.name} ${icons[store.store.id]? icons[store.store.id] : ""}</span></a><br>`);
                ...
                .then ((responseData) => {
                    if(responseData.count > 0){
                      ...
                      <h2 class="rating mt-3">TRAILER</h2>
                        <video controls style="width:90vw">
      ...
      fetch(`https://api.rawg.io/api/games?key=${process.env.API_KEY}&developers=${developers[0].id}&page_size=6&dates=2015-12-01,2022-12-31&ordering=-released`)
                .then((response) => response.json())
                .then ((responseData) => {
                    if(responseData.count > 0){
